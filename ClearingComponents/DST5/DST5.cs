...
namespace SF.Expand.Switch.Clearing
{
    public class DST5 : OrchPipeComponent
    {
        ...
        private bool ProcessTransaction(OrchPipeComponent.ComponentState State, string line)
        {
            ...
            switch (TIPREG)
            {
                // if TIPREG=1 -> operações com cartões
                case "1":
                // if TIPREG=2 -> operações detinadas a nibs
                case "2":
                ...
                default:
                    ...
                    State.OrchWrkData.GetWrkData().WriteNodeValue("TextoErroTrn", "Tipo registo " + TIPREG + " no DST5 não suportado", true);
                    break;
            }
            return result;
        }
